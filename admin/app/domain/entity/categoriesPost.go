...
//CategoriesPostSave strcut
type CategoriesPostSaveDTO struct {
	ID               uint64 `gorm:"primary_key;auto_increment" json:"id"`
...
//BeforeSave init
func (f *CategoriesPost) BeforeSave() {
	f.Name = html.EscapeString(strings.TrimSpace(f.Name))
}

//Prepare init
func (f *CategoriesPost) Prepare() {
	f.Name = html.EscapeString(strings.TrimSpace(f.Name))
...
func (f *CategoriesPost) ValidateV1() map[string]string {
...
func (f *CategoriesPost) Validate() map[string]string {
	var (
