...
func InitPost(pApp services.PostAppInterface, catPostApp services.CategoriesPostAppInterface,
	catPostsJoinApp services.CategoryPostsJoinAppInterface, langApp services.LanguageAppInterface, uApp services.UserAppInterface) *Post {
	return &Post{
		postApp:           pApp,
		categoriesPost:    catPostApp,
		categoryPostsJoin: catPostsJoinApp,
		userApp:           uApp,
		languageApp:       langApp,
	}
}
...
func (access *Post) Create(c *gin.Context) {
	stncsession.IsLoggedInRedirect(c)
	cats, _ := access.categoriesPost.GetAll()
	viewData := pongo2.Context{
		"title":    "İçerik Ekleme",
		"catsData": cats,
		"csrf":     csrf.GetToken(c),
...
func (access *Post) Store(c *gin.Context) {
	...
	stncupload.NewFileUpload().UploadFile(filenameForm, c.PostForm("Resim2"))
	...
	*/
	...
	catsData, _ := access.categoriesPost.GetAll()
	// var list []entity.CategoriesPostSaveDTO
	...
	if len(savePostError) == 0 {
		...
		var catPost = entity.CategoryPostsJoin{}
		for _, row := range catsPost {
			...
			saveCat, _ := access.categoryPostsJoin.Save(&catPost)
...
func (access *Post) Edit(c *gin.Context) {
	...
	if postID, err := strconv.ParseUint(c.Param("postID"), 10, 64); err == nil {
		...
		catsPostData, _ := access.categoryPostsJoin.GetAllforPostID(postID)
...
func (access *Post) Update(c *gin.Context) {
	...
	if len(savePostError) == 0 {
		...
		var catPost = entity.CategoryPostsJoin{}
		access.categoryPostsJoin.DeleteForPostID(idN)
		for _, row := range catsPost {
			catID, _ := strconv.ParseUint(row, 10, 64)
			catPost.CategoryID = catID
			catPost.PostID = saveData.ID
...
func (access *Post) Upload(c *gin.Context) {
	...
	stncupload.NewFileUpload().UploadFile(filenameForm, "")
