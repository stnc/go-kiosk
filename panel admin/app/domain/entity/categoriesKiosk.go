...
//TODO: ilerde sub CategoriesKiosk gelebilir

//CategoriesKiosk strcut
type CategoriesKiosk struct {
...
//CategoriesKioskSave strcut
type CategoriesKioskSaveDTO struct {
	ID               uint64 `gorm:"primary_key;auto_increment" json:"id"`
...
//BeforeSave init
func (f *CategoriesKiosk) BeforeSave() {
	f.Name = html.EscapeString(strings.TrimSpace(f.Name))
}

//Prepare init
func (f *CategoriesKiosk) Prepare() {
	f.Name = html.EscapeString(strings.TrimSpace(f.Name))
...
func (f *CategoriesKiosk) ValidateV1() map[string]string {
...
func (f *CategoriesKiosk) Validate() map[string]string {
	var (
